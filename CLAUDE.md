...
## Rules that matter
- Be gentle with EA: only syncs may call EA (never a page load, set open or solve); everything is cached with `fetchedAt`; sync only when stale (club and SBCs once a day after the 20:01 Europe/Bucharest drop, or on the first web app visit after it). The extension relays what the web app loads (`server/events.ts`), which fills the cache for free. Every call is metered per account (`server/meter.ts`, daily cap `EA_DAILY_LIMIT`, 15 min pause on throttling codes).
