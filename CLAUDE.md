...
- Auth: the UI sends `X-Account-Key`. Client mode (extension 0.7+): every EA request runs in the web app tab (`extension/hook.js` recipes, `server/jobs.ts` queue); the server keeps no SID and only calls EA once to prove a new account (`/api/hello`). Legacy mode (old extensions) still syncs server-side; remove it once all accounts run 0.7. Never add a job recipe that writes to EA. Keys come via URL fragment `#keys=` into `localStorage`.
