...
def solve(p):
    ...
    if p.get("needsChem"):
        ...
        for par in PARAMS:
            ...
            for g, members in groups.items():
                total = sum(v * inpos[i] for i, v in members)
                prev = None
    ...
    # Costs are floats; CP-SAT wants integers.
    m.Minimize(sum(int(round(pl["cost"] * 100)) * used[i] for i, pl in enumerate(players)))
    ...
    return {
        ...
        "slots": slots,
        "cost": solver.ObjectiveValue() / 100,
